...
interface OrganizationRequiredCardProps {
  userEmail: string;
  userRole: string;
  organizationId: string | null;
}

export const OrganizationRequiredCard: React.FC<OrganizationRequiredCardProps> = ({
  userEmail,
  userRole,
  organizationId,
}) => {
      ...
      <CardContent className="text-center space-y-4">
        ...
        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg text-left">
          ...
          <div className="space-y-1 text-sm text-blue-800 dark:text-blue-200">
            ...
            <div><strong>Organization:</strong> {organizationId || 'Not assigned'}</div>
          </div>
        </div>
      </CardContent>
    </Card>
